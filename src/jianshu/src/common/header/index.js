...
// 当把constructor函数去掉之后，组件中只剩下一个render函数，这样就变成了一个无状态组件
...
//               in={this.props.focused} // 控制入场和出场动画,就是true或false的值
  ...
  return (
    <HeaderWrapper>
      ...
      <Nav>
        ...
        <SearchWrapper>
          <CSSTransition // 设置搜索框动画
            in={props.focused} // 控制入场和出场动画,就是true或false的值
            timeout={200}  // 动画时长200ms
            classNames="slide" // 当是字符串时{}也可以不写,后面SearchWrapper中的.slide-enter 等类因为它起作用
            // 上面的动画设置其实已经完了，在网页面挂载NavSearch之前会添加几个样式，进场时样式名字分别为 slide-enter slide-enter-active
            // 离场时样式分别为 slide-exit slide-exit-active
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    handleInputFocus() {
      ...
      dispatch(action); // 运用reducer合并后，现在有一点不明白：是不是 dispatch(action) 会将 state 和action 加载到所有的 reducer 中呢？
    },
    // 输入框失去焦点时执行的函数
    handleInputBlur() {
